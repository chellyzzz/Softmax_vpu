...
class ICache(ADDR_WIDTH: Int = 32,
  DATA_WIDTH: Int = 32,
  CACHE_SIZE: Int = 16,
  WAY_NUMS: Int = 2,
  BYTES_NUMS: Int = 8
) extends Module with Parameter {
  ...
  val cache_valid = Reg(Vec(WAY_NUMS, Bool()))
  ...
  val hit  = cache_valid(hit_index) && (cache_tag(hit_index) === hit_tag)
